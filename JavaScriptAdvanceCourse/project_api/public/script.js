        ...
        addToBasket(id) {
            ...
            this.postData(`${API_URL}cart`, data).then(response => {
                if (response.status == 201) {
                    const idxGood = this.showcase.findIndex(good => good.id_product == id);
                    const good = this.showcase[idxGood]
                    const idxCart = this.cart.findIndex(stack => stack.id_product == id);
                    if (idxCart >= 0) {
        ...
        removeFromBasket(id) {
            this.postData(`${API_URL}cart/remove`, { id_product: id }).then(
                response => {
                    if (response.status == 201) {
                        const idxGood = this.showcase.findIndex(good => good.id_product == id);
                        const idxStack = this.cart.findIndex(stack => stack.id_product == id);
