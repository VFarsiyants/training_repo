let mover = {
    ...
    getDirection() {
        ...
        // VF: modified, added new diagonal available directions
        const availableDirections = [1, 2, 3, 4, 6, 7, 8, 9];

        while (true) {
            // Получаем от пользователя направление.
            // VF: prompt alert modified, new directions added
            let direction = parseInt(prompt('Введите число (1, 2, 3, 4, 6, 7, 8, 9), \
            куда вы хотите переместиться, "Отмена" для выхода.'));
            if (isNaN(direction)) {
                return null;
            }
            // Если направление не одно из доступных, то сообщаем что надо 
            // ввести корректные данные
            // и начинаем новую итерацию.
            if (!availableDirections.includes(direction)) {
                //VF: alert modified, new directions added
    ...
    getNextPosition(direction) {
        ...
        // VF: modified, added diagonal possible directions
        switch (direction) {
            case 1:
                nextPosition.y++;
                nextPosition.x--;
